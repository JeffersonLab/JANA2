...
string GetRevision(void)
{
	...
	//return "svn revsion 14114";

	// Extract revision number using svn and HALLD_HOME
	string revision = "";
	const char *HALLD_HOME = getenv("HALLD_HOME");
	if(HALLD_HOME != NULL){
		char cmd[1024];
		sprintf(cmd, "svn info %s | grep \"Revision:\" | awk '{print $2}' > .svn_revision", HALLD_HOME);
		system(cmd);
	}
	ifstream ifstr(".svn_revision");
	ifstr >> revision;
	ifstr.close();
	
	return string("svn revision ") + revision;
}
