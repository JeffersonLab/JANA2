...
struct Logger {
    JLogLevel level = JLogLevel::OFF;
    std::ostream *destination = &std::cout;

    Logger() {};
    Logger(JLogLevel level) : level(level) {}
    Logger(JLogLevel level, std::ostream* dest) : level(level), destination(dest) {};
    Logger(const Logger & logger) : level(logger.level), destination(logger.destination) {}
    ...
    static Logger everything() {
        return Logger(JLogLevel::TRACE);
    }

    static Logger nothing() {
        return Logger(JLogLevel::OFF);
    }
};
...
struct JLogMessage {
    ...
    JLogMessage(Logger* logger_, JLogLevel level_) :
...
// Logging for when you don't have a Logger readily available
...
public:
    ...
    Logger get_logger() {
        return Logger(_global_log_level);
    }

    Logger get_logger(std::string className) {
    ...
    static Logger logger(std::string className) {

        if (serviceLocator != nullptr) {
            auto loggingService = serviceLocator->get<LoggingService>();
