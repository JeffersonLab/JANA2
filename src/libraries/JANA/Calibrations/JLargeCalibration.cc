...
JLargeCalibration::JLargeCalibration(std::shared_ptr<JParameterManager> params, JCalibration *jcalib, string resource_dir) {
    ...
    // Get list of existing namepaths so we can check if they exist without JCalibration subclass printing errors.
    jcalib->GetListOfNamepaths(calib_namepaths);

    // Derive location of resources directory on local system. This can be specified in several ways, given here in
    // order of precedence:
    //
    ...
    //
    // Note that in nearly all instances, no second argument should be passed to the constructor so that the value can be changed
    ...
    if (params != nullptr) {
        ...
        if (params->Exists("JANA:RESOURCE_DEFAULT_PATH")) {
            p = params->GetParameter("JANA:RESOURCE_DEFAULT_PATH", RESOURCE_DEFAULT_PATH);
        } else {
            p = params->SetParameter("JANA:RESOURCE_DEFAULT_PATH", RESOURCE_DEFAULT_PATH);
        }
    ...
    // 2.
    if (params != nullptr) params->SetDefaultParameter("JANA:RESOURCE_DIR", this->resource_dir);
    ...
    if (params != nullptr) {
        if (params->Exists("JANA:RESOURCE_URL")) {
            overide_URL_base = true;
            URL_base = "";
            params->SetDefaultParameter("JANA:RESOURCE_URL", URL_base,
                                        "Base URL to use for retrieving resources. If set, this will override any URL_base values found in the calib DB.");
        }
    }
    ...
    curl_args = "--insecure";
    if (params)
        params->SetDefaultParameter("JANA:CURL_ARGS", curl_args,
                                    "additional arguments to be passed to the external curl program");
#endif // HAVE_CURL

    // Allow user to turn off checking of md5 checksums
    check_md5 = true;
    if (params)
        params->SetDefaultParameter("JANA:RESOURCE_CHECK_MD5", check_md5,
                                    "Set this to 0 to disable checking of the md5 checksum for resource files. You generally want this check left on.");
}
...
string JLargeCalibration::GetResource(string namepath) {
    ...
    if (jcalib) {
        ...
        if (has_URL_base || has_path) {
            ...
            string mess =
                    string("Neither \"URL_base\",\"path\" pair nor \"URL\" exist in DB for resource \"") + namepath +
                    "\" !";
            throw JException(mess);
        }
...
// The following will make all neccessary sub directories in order for the specified path to exist. It was
// taken from here:
