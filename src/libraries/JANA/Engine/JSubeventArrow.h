...
void JSplitArrow<InputT, OutputT>::execute(JArrowMetrics& result, size_t location_id) {
    ...
    if (in_status == InQueue::Status::Finished) {
        set_upstream_finished(true);
        status = JArrowMetrics::Status::Finished;
    }
    else if (in_status == InQueue::Status::Ready && out_status == OutQueue::Status::Ready) {
...
void JSubeventArrow<InputT, OutputT>::execute(JArrowMetrics& result, size_t location_id) {
    ...
    JArrowMetrics::Status status;
    if (in_status == SubeventQueue::Status::Finished) {
        set_upstream_finished(true);
        status = JArrowMetrics::Status::Finished;
    }
    else if (in_status == SubeventQueue::Status::Ready && out_status == JMailbox<SubeventWrapper<OutputT>>::Status::Ready) {
        status = JArrowMetrics::Status::KeepGoing;
    }
    else {
        status = JArrowMetrics::Status::ComeBackLater;
